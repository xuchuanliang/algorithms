...
中缀表达式的求值是我们人最熟悉的，但是对计算机来说却不好操作(前面我们讲的案例就能看的这个优先级问题)，因此，在计算结果时，往往会将中缀表达式转成其它表达式来操作(一般转成后缀表达式.)
### 后缀表达式（逆波兰表达式）
...
### 中缀表达式转后缀表达式的步骤
> 从上方前缀表达式、中缀表达式、后缀表达式的介绍中我们可以发现，其实前缀表达式和后缀表达式是反的，前缀表达式的运算是从右向左，后缀表达式的运算是从左向右，前缀和后缀都是符合
>计算机的运作方式；中缀表达式是符合人类的思维风格，但是中缀表达式在运算过程中需要处理很多情况，如优先级，左右括号等，效率会比较低，所以一般情况下是将中缀表达式转成后缀表达式，
>再使用后缀表达式进行运算。
>注意：由于后缀表达式和前缀表达式存在数字在中间没有通过运算符链接的情况，为了区分各个字符，一般我们默认各个字符之间使用空格隔开
...
4. 遇到运算符时， 比较其与 s1 栈顶运算符的优先级：
    ...
    3. 否则，即如果优先级相同或比s1栈顶元素的优先级低，将 s1 栈顶的运算符弹出并压入到 s2 中， 再次转到(4.1)与 s1 中新的栈顶运算符相比较；
5. 遇到括号时：
    1. 如果是左括号“(” ， 则直接压入 s1
